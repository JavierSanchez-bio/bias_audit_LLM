...
def evaluate_report_fingpt(report_text, system_prompt):
    try:
        response = client.chat.completions.create(
            model="fingpt", # Asegúrate de que este es el nombre del modelo en tu RunPod
            messages=[
...
for i in range(len(a)):
    ...
    final_results = []
    print(f"Starting FinGPT Organic Audit (Cycle {i+1}/7)...")
    ...
    filename = f"fingpt_organic_nocontext_round_{i+1}.csv"
